# 🏡 Indian House Price Prediction

####

An end-to-end **Machine Learning project** that predicts house prices in major Indian cities using an **XGBoost regression model**.  
The project includes a **Streamlit frontend** for user interaction.

---

## 🚀 Features
- Predict house prices based on location, property details, and amenities
- Handles categorical & numerical features using a trained ML pipeline
- Clean, user-friendly Streamlit interface
- Production-ready model using scikit-learn + XGBoost

---

## 🧠 Tech Stack
- **Python**
- **XGBoost**
- **scikit-learn**
- **Pandas & NumPy**
- **Streamlit** (Frontend)
- **Joblib** (Model persistence)
